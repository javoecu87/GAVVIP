...
def reservar():
    ...
    try:
        ...
        app.logger.debug(f"Datos recibidos: Origen={origen}, Destino={destino}, Fecha={fecha}, Hora={hora}, Personas={personas}")
    ...
    # Ejecuta la función asincrónica y maneja posibles errores
    try:
        asyncio.run(enviar_mensaje())
        app.logger.debug("Mensaje enviado a Telegram con éxito")
    except Exception as e:
        app.logger.error(f"Error al enviar mensaje a Telegram: {e}")
        return "Error al enviar el mensaje a Telegram", 500
